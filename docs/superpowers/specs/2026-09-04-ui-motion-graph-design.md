...
원본 프로젝트는 UI 연출을 MonoBehaviour "모듈"로 조립한다. 세 계열이 있다.
...
- **원본 프로젝트 코드 변경.** 원본 프로젝트는 "어떤 노드가 필요한가"를 정하는 **참조 원본**으로만 쓴다. 이식 여부는 패키지 완성 후 별도로 결정한다.
...
| 3 | 위상 트리거 | **이름 있는 트리거 채널**, `Start`/`Loop`/`End`는 예약 이름. 채널은 그래프 안의 `Trigger` **노드**로 저작하고 선언 목록은 거기서 파생된다 | 생명주기 고정으로는 클릭·보상 같은 이벤트 연출을 표현할 수 없다. 노드로 옮긴 이유는 4.4 참조 — 진실이 두 곳에 있으면 어긋나고, 진입점이 캔버스에 보이지 않는다 |
...
| 9 | 원본 프로젝트 | **참조 원본만** | 범위가 명확하고 운영 중인 게임에 리스크가 0 |
...
**트리거 목록은 저작하는 값이 아니라 파생되는 값이다.** 5.1의 슬롯 목록과 같은 논리다 — 사람은 캔버스에 `Trigger` 노드를 놓고 이름과 정책을 정할 뿐이고, 그래프가 자기 노드들을 훑어 목록을 만든다(`TriggerIntrospector`). 노드를 지우면 그 트리거도 사라진다.

`TriggerNode`가 **유일한 진실**이다. 예전에는 그래프가 `TriggerDeclaration` 목록(이름 · 정책 · 진입 노드 id)을 따로 들고, 캔버스의 `Trigger` 노드는 아무 일도 하지 않는 표식이었다. 진실이 두 곳에 있으면 어긋날 수 있고, 어긋나면 무엇이 맞는지 알 방법이 없다. 무엇보다 "이 노드가 `Start`의 진입점"이라는 사실이 캔버스에 보이지 않고 옆 패널에만 있었다.

- **진입점은 트리거 노드 자신이다.** 나가는 간선이 곧 실행 흐름이다. 예전처럼 "이름과 진입 노드 id"를 따로 들면 그 노드를 지웠을 때 목록에 죽은 id가 남는다.
- 이름은 앞뒤 공백을 잘라 대조한다. 이름이 비면 발사할 방법이 없으므로 목록에서 빠지고 경고가 난다.
- 같은 이름이 둘이면 **먼저 나온 것이 이긴다.** 뒤엣것은 경고와 함께 버려진다.
- 실행 의미는 바뀌지 않는다. `TriggerNode`는 즉시 완료하고 자식으로 흘려보내므로, 진입점이 그 노드가 되어도 아무것도 하지 않는 노드가 하나 앞에 붙을 뿐이다.
- `MotionRuntime.Fire("Start")` · `SubGraphNode.EntryTrigger` · 브릿지의 `Fire`/`WaitFor`는 전부 이름으로만 다루므로 영향이 없다.

옛 형식으로 저장된 그래프는 검사기가 오류로 알리고, 에디터의 `Window > UI Motion > Migrate Graphs`가 선언을 노드로 옮긴다. 조용히 트리거를 잃는 것이 가장 나쁜 결과이므로 그 경로는 만들지 않는다.
...
방치형 게임에서 앱은 몇 시간씩 켜져 있다. 꺼지지 않은 트윈 하나가 계속 돌고, 부유 연출이 만든 중간 위치를 다음 재생이 기준으로 삼으면 오브젝트가 갈수록 밀린다. 원본 프로젝트의 `UIFloatingModule`은 이 문제를 `_origin` 캡처와 `OnStop`의 복원으로 손수 해결하고 있다. 그 처리를 프레임워크가 모든 노드에 대해 보장하도록 승격한다.
...
설명을 별도 문서가 아니라 어트리뷰트에, 예시를 스크린샷이 아니라 **실행 가능한 그래프 에셋**에 두는 것이 핵심이다. 둘 다 코드 옆에 있어 썩지 않고, 툴이 기계적으로 검사할 수 있다. 팔레트에서 노드를 고르면 그 요약과 예시 그래프의 존재 여부가 뜨고, "예시 그래프 열기"로 그 예시를 그래프 창에 띄운다.

**그 자리에서 재생하지는 않는다(계획 3에서 정함).** 연출을 재생하려면 실제 씬 오브젝트와 슬롯 바인딩이 필요한데 팔레트에는 재생 대상이 없다. 씬에 임시 오브젝트를 만들어 재생하는 것은 프리팹으로 새어 나갈 위험이 커서 하지 않는다. 예시를 열어 실제 `MotionPlayer`에 적용하면 그때부터는 인에디터 프리뷰가 재생을 맡는다.
...
**사운드와 햅틱.** 프로젝트마다 오디오 시스템이 다르므로 코어에 넣으면 "참조 0"이 깨진다. `SendSignal("sfx:click")` 노드 하나만 두고, 신호를 받아 실제로 소리를 내거나 진동시키는 쪽은 프로젝트가 구현한다. 원본 프로젝트의 `UISoundModule` / `UIButtonHapticModule`이 여기 해당한다.
